...
from ansible_collections.community.mongodb.plugins.module_utils.basic import _remove_values_conditions
NoneType = type(None)
############
...
def insert_document(client, database, collection, document):
    ...
    if "_id" not in document.keys():
        inserted_id = "%s" % str(client[database][collection].insert_one(document).inserted_id)
        status = True
    else:
